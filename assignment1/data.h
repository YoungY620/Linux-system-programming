...
#define LINES 30        // DO NOT CHANGE
#define COLS 200        // DO NOT CHANGE
...
#define PL_C_UPR (COLS - PL_WIDTH)              // upper band
#define PL_L_UPR (LINES - PL_HIGHT)
#define PL_C_LWR 0                              // lower band
#define PL_L_LWR 0

#define BLT_C_UPR (COLS - BLT_WIDTH)            // bullet coor upper band
#define BLT_L_UPR (LINES - BLT_HIGHT - PL_HIGHT)
...
int get_reverse(struct status *to, const struct status *from){
    ...
    if(from->o_bullets != NULL){
        free(to->s_bullets);
        to->s_bullets = NULL;
        to->s_bullets = malloc(sizeof(struct pos)*to->s_bullsize);
        memcpy(to->s_bullets, from->o_bullets, sizeof(struct pos)*to->s_bullsize);
    }
    if(from->s_bullets != NULL){
        free(to->o_bullets);
        to->o_bullets = NULL;
        to->o_bullets = malloc(sizeof(struct pos)*to->o_bullsize);
...
int str_to_data(const char *str, struct status *sts,int s_size, int o_size){
    ...
    if(s_size < sts->s_bullsize){
        fprintf(stderr,"not enough s_size.");
        return -1;
    }
