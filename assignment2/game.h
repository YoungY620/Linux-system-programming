...
void check_health(struct status *stsa, int *res_code){
    ...
    *res_code = -1;
    for(i=0; i<stsa->s_bullsize; i++){
        if(stsa->s_bullets[i].l == stsa->other.l+1
    ...
    for(i=0; i<stsa->o_bullsize; i++){
        if(stsa->o_bullets[i].l == stsa->self.l-1
...
int periodic_step(struct status *stsa,struct status *stsb, int *res_code){
    int i;
    s_bullet_step(&(stsa->s_bullets),&(stsa->s_bullsize));
    o_bullet_step(&(stsa->o_bullets),&(stsa->o_bullsize));
...
void do_shoot(struct status *self){
    ...
    struct pos bullet;
    bullet.c = self->self.c+(int)(PL_WIDTH/2)+1;
    bullet.l = (self->self.l-1);
    // if there is bullets
    if(self->s_ep == 0){
        return;
    }
